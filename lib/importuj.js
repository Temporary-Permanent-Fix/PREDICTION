...
export function prevodManhours(ws, XLSX) {
  ...
  for (const r of rows) {
    ...
    const h = +r["SJL/ManHours"];
    const rok = +r["Date – Year"], mes = MESIACE[String(r["Date – Month"] ?? "").trim()], den = +r["DayOfMonth"];
    if (!proc || !isFinite(h) || !rok || !mes || !den) continue;
    const d = `${rok}-${pad(mes)}-${pad(den)}`;
    const k = `${d}|${PROC_MAP[proc] || proc}`;
    acc.set(k, (acc.get(k) || 0) + h);
  }
  if (!acc.size) throw new Error("ManHours: nenašli sa riadky s hodinami.");
  const riadky = [...acc.entries()].map(([k, v]) => { const [d, p] = k.split("|"); return [d, p, Math.round(v * 100) / 100]; })
    .sort((a, b) => (a[0] === b[0] ? (a[1] < b[1] ? -1 : 1) : a[0] < b[0] ? -1 : 1));
  const dni = [...new Set(riadky.map((r) => r[0]))];
  const procesy = [...new Set(riadky.map((r) => r[1]))];
  return {
    subory: { "manhours.csv": csv(["datum", "proces", "hodiny"], riadky) },
