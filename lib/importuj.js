...
export function detekuj(ws, XLSX) {
  ...
  const vzorka = mriezka.slice(1).map((r) => txt(r, STLPCE.vq.proces));
...
export function prevodVolumes(ws, XLSX) {
  ...
  const rows = vsetky.filter((r) => txt(r, STLPCE.vq.proces).startsWith("Výtlak |") && bunka(r, STLPCE.vq.smena) != null);
  if (!rows.length) throw new Error("VOLUMES: nenašli sa riadky s procesmi „Výtlak | …“.");
  const den = (r) => iso(excelDate(bunka(r, STLPCE.vq.smena)));
  const smena = (r) => iso(excelDate(bunka(r, STLPCE.vq.smena)));
  ...
  for (const [proces, nazov] of [["Výtlak | 1. Received", "prijem_hodinove.csv"], ["Výtlak | 6. Sorted", "baseline_hodinove.csv"]]) {
    const m = new Map();
    for (const r of rows) {
      if (txt(r, STLPCE.vq.proces) !== proces) continue;
      const h = cislo(r, STLPCE.vq.hodina);
      if (!Number.isInteger(h) || h < 0 || h > 23) continue;
      const k = `${den(r)}#${h}`;
      m.set(k, (m.get(k) || 0) + cislo(r, STLPCE.vq.celkem));
    }
    if (!m.size) continue;
    const riadky = [...m.entries()]
  ...
  for (const r of rows) {
    ...
    const o = dni.get(d);
    o[txt(r, STLPCE.vq.proces)] = (o[txt(r, STLPCE.vq.proces)] || 0) + cislo(r, STLPCE.vq.celkem);
  }
...
export function prevodQuality(ws, XLSX) {
  const vsetky = XLSX.utils.sheet_to_json(ws, { raw: true, cellDates: true });
  const rows = vsetky.filter((r) => txt(r, STLPCE.vq.proces).startsWith("Kvalita |") && bunka(r, STLPCE.vq.smena) != null);
  if (!rows.length) throw new Error("QUALITY: nenašli sa riadky s procesmi „Kvalita | …“.");
  const POZDE = "Pozdě dokončené vše";
  const smena = (r) => iso(excelDate(bunka(r, STLPCE.vq.smena)));
  const proc = (r) => txt(r, STLPCE.vq.proces).replace("Kvalita | ", "");
  ...
  for (const r of rows) {
    ...
    o.c += cislo(r, STLPCE.vq.celkem); o.z += cislo(r, STLPCE.vq.pozde);
    den.set(k, o);
  }
  ...
  for (const r of rows) {
    const d = smena(r);
    if (d <= cut) continue;
    const p = proc(r), h = cislo(r, STLPCE.vq.hodina);
...
export function prevodManhours(ws, XLSX) {
  ...
  const acc = new Map();
  for (const r of rows) {
    const proc = txt(r, STLPCE.manhours.proces);
    if (!proc) continue;
    const nazov = PROC_MAP[proc] || proc;
    const dt = bunka(r, STLPCE.manhours.datum);
    const d = dt instanceof Date ? iso(dt) : (typeof dt === "number" ? iso(excelDate(dt)) : null);
    if (!d) continue;
    const mh = cislo(r, STLPCE.manhours.manhours);
    const jbl = cislo(r, STLPCE.manhours.joblines);
    const k = `${d}|${nazov}`;
